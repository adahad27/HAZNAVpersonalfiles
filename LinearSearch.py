...
def polarConversion(xCoord, yCoord):
...
data = pd.read_csv(r"\\wsl.localhost\Ubuntu\home\adahad27\UMRHELab\HAZNAV\testfileout1.csv", index_col= "coordinates")
...
while(data.iloc[drone.xCoord, drone.yCoord] != 2000): #this is hard coded right now, we will have to change this to a tolerance value because this is the main loop
    ...
    coordinates = np.concatenate((coordinates, currCoords,eastCoords, southCoords, southeastCoords)) #the argument has to be passed in as a tuple idk why, probably some interface thing to make sure that nothing is modified?
    ...
    incrementArray = polarConversion(results.x[1], results.x[2])
